...
## 铁律：提升点一律项目级（2026-09-21 作者指令）

> 只要是提升的点（写作方法、规则定义、判定口径、流程修正），一律**项目级全局应用**：
> 落到 `assets/rules`／`templates`／`hooks`／scripts，让所有书继承——**禁止只修出问题的那一本**。
> 例外仅限：本质是本书专属校准值（如某书按自己样板书实测的节奏阈值）。
> 流程：在书里发现 → 抽象成通用规则 → 落 kit 层 → 四件套验证 → 单本书同步应用。
